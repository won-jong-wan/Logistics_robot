...
//	if(GPIO_Pin == sensor_input_X_Pin)  //8번 핀
//	  {
//		  GPIOB->ODR ^= 1 << 7;
//
//		  photo_in_encoder_X = photo_out_encoder_X;
//		  photo_out_encoder_X = encoder_count_x;
//
//	  }
	...
	if(HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_1)==1)  //8번 핀
		  {
			//  EXTI8_flag=1;
...
/*
	if(GPIO_Pin == GPIO_PIN_8 && HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_8) == GPIO_PIN_SET)
	{ // 라이징
	     GPIOB->ODR &= ~1 << 7;
	     photo_in_encoder_X=   encoder_count_x;
	}
	else if (GPIO_Pin == GPIO_PIN_8 && HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_8) == GPIO_PIN_RESET)
    { // 폴링
...
//	{
//	        if (HAL_GPIO_ReadPin(EXTI_GPIO_PORT, EXTI_GPIO_PIN) == GPIO_PIN_RESET && !fallingEdgeHandled) {
//	            // Handle falling edge interrupt here
//	          fallingEdgeHandled = 1; // Set flag to indicate falling edge has been handled
//	        }
//	        else if (HAL_GPIO_ReadPin(EXTI_GPIO_PORT, EXTI_GPIO_PIN) == GPIO_PIN_SET && !risingEdgeHandled) {
//	            // Handle rising edge interrupt here
//	            risingEdgeHandled = 1; // Set flag to indicate rising edge has been handled
//	        }
//	    }
