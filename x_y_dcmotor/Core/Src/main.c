...
#define PHY_POWERDOWN ((uint16_t)0x0800U) /*!< 절전 모드 선택 */
...
uint32_t encoder_count_y = 10000;
...
extern int tim11_flag;

extern int tim12_flag;
...
//	HAL_TIM_PWM_Start_IT(&htim3, TIM_CHANNEL_1);
  ...
  /* USER CODE BEGIN WHILE */
	...
	//	SYSCFG_MEMRMP->SWP_FMC |= 1<<6;
	//	SWP_FMC
 //ETH_r

	//	PHY_RESET

		encoder_count_x = TIM2->CNT;
		encoder_count_y = TIM4->CNT;

		//TIM3->CCR2=qwedasd;

		//바퀴 pid >> tim6 인터럽트 핸들러 내부
		//z축 볼스크류 스텝모터  >> tim11 인터럽트 핸들러 내부
		//축 스텝모터  >> tim13 인터럽트 핸들러 내부
		/*
		 ...
		 if (tim13_flag == 1 && step_enable == 1) { //축 스텝모터

		 if (tim14_flag >= 120) { //1.2초후 최고속도
		 shaft_pulse_cycle = 9000;
		 }
		 ...
		 */
		//  printf("%d %d \n\r",encoder_count,input_position );
//	  HAL_Delay(1);
	}
  /* USER CODE END 3 */
}
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0|test_Pin|test2_Pin|LD3_Pin
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOG, DC_M_GO_X_Pin|GPIO_PIN_2|GPIO_PIN_3|side_pin_test_Pin
                          |testG10_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : PB0 test_Pin test2_Pin LD3_Pin
                           LD2_Pin */
  GPIO_InitStruct.Pin = GPIO_PIN_0|test_Pin|test2_Pin|LD3_Pin
                          |LD2_Pin;
  ...
  /*Configure GPIO pins : DC_M_GO_X_Pin PG2 PG3 side_pin_test_Pin
                           testG10_Pin */
  GPIO_InitStruct.Pin = DC_M_GO_X_Pin|GPIO_PIN_2|GPIO_PIN_3|side_pin_test_Pin
                          |testG10_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
	if (htim->Instance == TIM13) {
		...
		/* tim13_flag=1;
		 ...
		 if(step_pulse_count==200*10)  //    2000/rev
		 {
